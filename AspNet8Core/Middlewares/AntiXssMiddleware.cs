...
namespace AspNet8Core.Middlewares;
...
public class AntiXssMiddleware
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        if (!string.IsNullOrWhiteSpace(context.Request.QueryString.Value))
        {
            ...
            {
                //await RespondWithAnError(context).ConfigureAwait(false);
                RespondRedirectWithAnError(context);
                return;
            }
        }
        ...
        try
        {
            ...
            var decodedBody = WebUtility.UrlDecode(content);
            if (CrossSiteScriptingValidation.IsDangerousString(decodedBody, out _))
            {
                RespondRedirectWithAnError(context);
                return;
            }
    ...
    private void RespondRedirectWithAnError(HttpContext context)
    {
        context.Response.Clear();
        context.Response.Headers.AddHeaders();
        context.Response.StatusCode = _statusCode;
